...
void T1_Init(){
	...
	// prescale en mS
	conf_timer.PrescaleOption = TIM_PRESCALE_USVAL;
	conf_timer.PrescaleValue = 100000;

	// timer1
	conf_interrupt.MatchChannel=1;
	// enable:interruption sur match
	conf_interrupt.IntOnMatch=ENABLE;
	// remise a zero en match
	conf_interrupt.ResetOnMatch=ENABLE;
...
void pin_Configuration()
{
		...
		int hpBit = (1<<9);
 		// Port
		int joystickPort[6] = {2, 2, 2, 2, 2, 1};
		// Pin
		int joystickPin[6] = {10, 12, 13, 8, 11, 21};

		// Bouton Menu : Entrée
		FIO_SetDir(2, (1<<11), 0);
		// Bouton Menu : Utilisable
		FIO_SetMask(2, (1<<11), 0);

		// Led0 : Sortie
		FIO_SetDir(0, ledBit, 1);
		// Led0 : Utilisable
		FIO_SetMask(0, ledBit, 0);

		// Haut-Parleur : Sortie
		FIO_SetDir(1, hpBit, 1);
		// Haut-Patleut : Utilisable
		FIO_SetMask(1, hpBit, 0);
		...
		// GPIO0
		memory27.Portnum = PINSEL_PORT_0;
		...
		// GPIO0
		memory28.Portnum = PINSEL_PORT_0;
		...
		{
			// joystick : Entrée
			FIO_SetDir(joystickPort[i], joystickPin[i], 0);
			// joystick : Utilisable
			FIO_SetMask(joystickPort[i], joystickPin[i], 0);
		}
}
...
int main(void)
{
	...
	lcd_Initializtion();
	// init pinsel tactile et init tactile
	touch_init();
	// init pinsel memoire, buzzer, led
	pin_Configuration();
	// init mémoire
	init_i2c_eeprom();
	// init timer musique
	...
	{
 		// Si la memoire est invalide ou on appuie sur la réinitialisation
		clean_memory();
		create_gamekey();
	}
	...
	while(1)
	{
		...
		if (menu == -1)
		{
			...
			if (mapLoad)
			{
				...
				// On le fait qu'une seule fois pour une question de rapidité d'execution
				pLife = 0;
			}
			...
			if (dir == 5)
			{
				menu = 1;
				mapLoad = true;
				// Préparation de la sauvegarde "data"
							...
							&& touch_y >= 2650 && touch_y <= 3500)
					{
						...
						if (check_save(2))
						{
							load_save(numSave, data);
 							// Chargement de la sauvegarde "data"
							filldown_save(data, numSave, &life, &mapX, &mapY, &pX, &pY, 0);
						}
						else
						{
